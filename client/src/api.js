...
export const api = {
  ...
  async getSupplierStats(supplierId, includeHistory, opts) {
    const params = new URLSearchParams();
    if (supplierId) {
    ...
    if (opts && opts.warehouse && opts.warehouse !== "all") {
      params.append("warehouse", opts.warehouse);
    }
    const response = await axios.get(
      API_BASE_URL + "/suppliers/stats?" + params.toString(),
    );
